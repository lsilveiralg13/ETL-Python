...
# =========================
#   Inscrição Estadual (IE) - CNPJá (API gratuita)
# =========================

def _cnpja_pick_ie(payload: Dict[str, Any], uf_preferida: Optional[str] = None) -> Dict[str, Any]:
    """
    Tenta extrair IE do payload da CNPJá (open).
    Retorna sempre um dict padronizado.
    """
    uf_preferida = (uf_preferida or "").strip().upper()

    possiveis_listas: List[Any] = []

    # chaves candidatas no topo
    for k in ("inscricoes_estaduais", "inscricoesEstaduais", "inscricoes", "inscricoes_estaduais_sintegra"):
        v = payload.get(k)
        if isinstance(v, list):
            possiveis_listas.extend(v)

    # chaves candidatas dentro de sub-blocos comuns
    for container_key in ("estabelecimento", "cadastro_contribuinte", "cadastroContribuinte"):
        cont = payload.get(container_key)
        if isinstance(cont, dict):
            for k in ("inscricoes_estaduais", "inscricoesEstaduais", "inscricoes"):
                v = cont.get(k)
                if isinstance(v, list):
                    possiveis_listas.extend(v)

    if not possiveis_listas:
        return {
            "ie": None,
            "uf_ie": None,
            "situacao_ie": None,
            "fonte": "CNPJá (open)",
            "erro": "Nenhuma inscrição estadual encontrada no retorno",
        }

    def norm_uf(x: Any) -> str:
        return str(x or "").strip().upper()

    def norm_ie(x: Any) -> str:
        return str(x or "").strip()

    candidatos = []
    for item in possiveis_listas:
        if not isinstance(item, dict):
            continue

        ie = (
            item.get("inscricao_estadual")
            or item.get("ie")
            or item.get("numero")
            or item.get("inscricao")
            or item.get("value")
        )
        uf = item.get("uf") or item.get("estado") or item.get("sigla_uf")
        situ = item.get("situacao") or item.get("status") or item.get("situacao_cadastral") or item.get("descricao_situacao")

        ie_s = norm_ie(ie)
        uf_s = norm_uf(uf)
        situ_s = str(situ or "").strip() or None

        if ie_s:
            candidatos.append((uf_s, ie_s, situ_s))
    ...
    if not candidatos:
        return {
            ...
            "situacao_ie": None,
            "fonte": "CNPJá (open)",
            "erro": "Lista de IE presente, mas nenhum número de IE foi identificado",
        }

    # prioriza UF informada
    if uf_preferida:
        for uf_s, ie_s, situ_s in candidatos:
            if uf_s == uf_preferida:
                return {
                    "ie": ie_s,
                    "uf_ie": uf_s or uf_preferida,
                    "situacao_ie": situ_s,
                    "fonte": "CNPJá (open)",
                    "erro": None,
                }

    # fallback: primeira IE disponível
    uf_s, ie_s, situ_s = candidatos[0]
    return {
        "ie": ie_s,
        "uf_ie": uf_s or None,
        "situacao_ie": situ_s,
...
def fetch_ie_cnpja_open(
    cnpj: str,
    uf_preferida: Optional[str] = None,
    timeout: int = 20,
) -> Dict[str, Any]:
    """
    Busca IE via CNPJá (API gratuita).
    Retorna dict padronizado: ie, uf_ie, situacao_ie, fonte, erro.
    """
    cnpj_d = only_digits(cnpj)
    url = f"https://open.cnpja.com/office/{cnpj_d}"

    try:
        r = requests.get(url, timeout=timeout, headers={"User-Agent": "BIA-CNPJ-Streamlit/1.0"})
        if r.status_code == 404:
            return {"ie": None, "uf_ie": None, "situacao_ie": None, "fonte": "CNPJá (open)", "erro": "CNPJ não encontrado na CNPJá"}
        r.raise_for_status()
        payload = r.json()
        return _cnpja_pick_ie(payload, uf_preferida=uf_preferida)
    except requests.RequestException as e:
        return {"ie": None, "uf_ie": None, "situacao_ie": None, "fonte": "CNPJá (open)", "erro": str(e)}
