...
STAGING_TABLE_NAME = 'staging_financeiro_multimarcas' # Nome da tabela ajustado para refletir os dados de parceiros

# Mapeamento de colunas do Excel para o MySQL e seus tipos de dados SQLAlchemy
COLUMN_MAPPING_AND_TYPES = {
    'Cód. Parceiro': {'new_name': 'codigo_parceiro', 'type': Integer},
    'Razão Social': {'new_name': 'razao_social', 'type': String(255)},
    'CNPJ / CPF': {'new_name': 'cnpj_cpf', 'type': String(20)},
    'Nome Parceiro': {'new_name': 'nome_parceiro', 'type': String(255)},
    'Dt Cadastro': {'new_name': 'data_cadastro', 'type': DateTime},
    'Dt Abertura': {'new_name': 'data_abertura', 'type': DateTime},
    'Tempo Abertura (em anos)': {'new_name': 'tempo_abertura_anos', 'type': Float},
    'Dt Análise Serasa': {'new_name': 'data_analise_serasa', 'type': DateTime},
    'Limite Sug. Serasa': {'new_name': 'limite_sugerido_serasa', 'type': Numeric(15, 2)},
    'Limite de credito utilizado': {'new_name': 'limite_credito_utilizado', 'type': Numeric(15, 2)},
    'limite de credito Total': {'new_name': 'limite_credito_total', 'type': Numeric(15, 2)},
    'Limite Cred. Disponível': {'new_name': 'limite_credito_disponivel', 'type': Numeric(15, 2)},
    'Bloq. Venda a Prazo': {'new_name': 'bloqueio_venda_prazo', 'type': String(5)}, # Sim/Não
    'Nome do Vendedor': {'new_name': 'vendedor', 'type': String(100)},
...
def run_etl():
    """
    Executa o pipeline ETL completo para carregar dados de parceiros para o MySQL.
    """
    df = None
    connection = None
    cursor = None
    ...
    try:
        ...
        date_cols = ['data_cadastro', 'data_abertura', 'data_analise_serasa']
        ...
        numeric_cols = ['limite_sugerido_serasa', 'limite_credito_utilizado', 'limite_credito_total', 'limite_credito_disponivel']
